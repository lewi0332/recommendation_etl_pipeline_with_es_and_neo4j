"""
This is a devolpment script to test the speed of the neo4j graph 
database and the neo4j python driver with the built-in 
Euclidean and Cosine similarity algorithms in Neo4j.
...
uri =  os.environ.get("AWS_BOLT") # bolt+routing:// protocol is used for the connection
...
next = time.time()
with graphDB_Driver.session() as graphDB_Session:
    image = graphDB_Session.run(
        f"MATCH (p:With_photos) WHERE p.igFollower > {min_followers} AND p.igFollower < {max_followers} \
            ...
            CALL algo.similarity.euclidean.stream(personIMAGE, {{sourceIds: sourceIds, topK:1000}}) \
            YIELD item1, item2, similarity \
            with algo.getNodeById(item1) AS from, algo.getNodeById(item2) as to, similarity \
            RETURN from.cardId as from, to.cardId as to, similarity \
            ORDER BY similarity"
    ).values()
    print(f"\n\nImage Euclidean Distance took this long: {time.time() - next:.3f}sec")
    next2 = time.time()
...
min = df_image["similarity"].min()
max = df_image["similarity"].max()
df_image["similarity"] = (df_image["similarity"] - min) / (max - min)
